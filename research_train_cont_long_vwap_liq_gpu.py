...
OUT_DIR = Path("/home/vitamind/my_project/model6/research_runs/2026-03-30-cont-long-vwap-liq-gpu")
...
FEATURES = [
    ...
    "liq_total_z96",
    "funding_delta_z96",
    "intraday_break_hi_atr",
    "intraday_close_vs_hi_atr",
    "intraday_range_atr",
    "session_asia",
    "session_london",
    "session_us",
...
class Config:
    ...
    def name(self) -> str:
        return (
            ...
            f"vw{int(self.vwap_thr*1000):03d}_"
            f"lz{int((self.liq_short_z_thr+1)*100):03d}_"
            f"h{self.horizon}_s{int(self.stop_atr*100):03d}_t{int(self.take_atr*100):03d}"
        )


FOLDS = [
    {"name": "2024-06-30->2025-06-30", "train_end": "2024-06-30T23:59:59Z", "val_end": "2025-02-28T23:59:59Z", "test_end": "2025-06-30T23:59:59Z"},
    {"name": "2024-10-31->2025-10-31", "train_end": "2024-10-31T23:59:59Z", "val_end": "2025-06-30T23:59:59Z", "test_end": "2025-10-31T23:59:59Z"},
    {"name": "2025-03-31->2026-03-01", "train_end": "2025-03-31T23:59:59Z", "val_end": "2025-09-30T23:59:59Z", "test_end": "2026-03-01T23:59:59Z"},
]


def add_extra_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["close_vs_vwap_atr"] = (out["close"] - out["vwap"]) / out["atr"].clip(lower=1e-8)
    out["vwap_slope_8"] = out["vwap"].pct_change(8).fillna(0.0)
    for col, zname in [
        ("liq_short", "liq_short_z96"),
        ("liq_total", "liq_total_z96"),
        ("funding_delta", "funding_delta_z96"),
    ]:
        mean = out[col].rolling(96, min_periods=24).mean()
        std = out[col].rolling(96, min_periods=24).std().replace(0, np.nan)
        out[zname] = ((out[col] - mean) / std).replace([np.inf, -np.inf], np.nan).fillna(0.0)

    day = out["timestamp"].dt.floor("D")
    intraday_hi = out.groupby(day)["high"].cummax().shift(1)
    intraday_lo = out.groupby(day)["low"].cummin().shift(1)
    out["intraday_break_hi_atr"] = (out["high"] - intraday_hi) / out["atr"].clip(lower=1e-8)
    out["intraday_close_vs_hi_atr"] = (out["close"] - intraday_hi) / out["atr"].clip(lower=1e-8)
    out["intraday_range_atr"] = (intraday_hi - intraday_lo) / out["atr"].clip(lower=1e-8)

    out["session_asia"] = ((out["hour"] < 8)).astype("int8")
    out["session_london"] = (((out["hour"] >= 8) & (out["hour"] < 16))).astype("int8")
    out["session_us"] = ((out["hour"] >= 16)).astype("int8")
    return out.replace([np.inf, -np.inf], np.nan)


def make_event(df: pd.DataFrame, cfg: Config) -> pd.Series:
    return (
        ...
        & (df["volume_delta"] > 0)
        & (df["macd_hist"] > 0)
        & (df["close_vs_vwap_atr"] >= cfg.vwap_thr)
        & (df["liq_short_z96"] >= cfg.liq_short_z_thr)
    ).fillna(False)
...
def optimize_quantile(scores, edge, cost_bps=20):
    best = None
...
def top_positive_buckets(frame, score_col, edge_col, thr, bucket, top_k):
    sel = frame[frame[score_col] >= thr].copy()
    if sel.empty:
        return set()
...
def select_session_filter(val_frame: pd.DataFrame, thr: float):
    ...
    for hk, dk in itertools.product([2, 3], [2, 3]):
...
def run_cfg(df: pd.DataFrame, cfg: Config):
    work = df.copy()
    ...
    for fold in FOLDS:
        ...
        if min(len(tr), len(va), len(te)) < 40:
            fold_results.append({"fold": fold["name"], "skipped": True, "n_train": int(len(tr)), "n_val": int(len(va)), "n_test": int(len(te))})
            continue
        ...
        va["score"] = model.predict(X_val)
        te["score"] = model.predict(X_test)
        best_q = optimize_quantile(va["score"].to_numpy(), va["edge_ret"].to_numpy(), cost_bps=20)
        if best_q is None:
            fold_results.append({"fold": fold["name"], "skipped": True, "n_train": int(len(tr)), "n_val": int(len(va)), "n_test": int(len(te))})
            continue
    ...
    score = float(np.nanmedian(test_comp)) + 0.4 * float(np.nanmean(test_avg)) + 0.8 * float(latest["test_backtest"].get("compounded_return_pct", -999))
    return {
        "config": asdict(cfg),
        "folds": fold_results,
        "robust_score": score,
