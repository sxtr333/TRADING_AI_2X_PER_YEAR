...
def main():
    ...
    leverage = 1.0
    trade_frac = 0.2  # 20% of equity per trade
    threshold_bump_sigma = 0.4  # tuned via grid search
    cooldown_steps = 32  # 32*15m = 8h cooldown per model
    max_concurrent = 1
    batch_size = 64
    ...
    # load best thresholds from reports
    ...
    print("Models used:")
    for s in specs:
        print(f"- {s.name}: {s.direction}, h{s.horizon}, thr={s.threshold:.6f}, strat={s.strategy}")
