...
def _load_model_keras3(model_path: str):
    ...
    custom_objects = {
        ...
        "model6>TimePositionalEncoding": TimePositionalEncoding,
...
def load_stats(path: str) -> dict:
    ...
    stats = {
        ...
        "val_start_ts": str(s["val_start_ts"][0]) if "val_start_ts" in s else None,
...
def main():
    ...
    # apply per-head scaling (if any) before bias-shift and metrics
    ...
    if args.bias_shift != "none":
        ...
        if isinstance(scale_map, dict) and scale_map:
            for h in pred_bias_by_h.keys():
                key = f"price_h{h}" if f"price_h{h}" in scale_map else "price"
                if key in scale_map:
                    pred_bias_by_h[h] = pred_bias_by_h[h] * float(scale_map[key])
