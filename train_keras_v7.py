...
def pick_feature_cols(df: pd.DataFrame, explicit: list[str] | None = None) -> list[str]:
    ...
    exclude_prefixes = ("target_", "label_", "tb_", "y_true_", "y_pred_", "future_", "meta_label_", "meta_y_")
    exclude_cols = {
        "timestamp",
        "label_3cls",
        "label_3cls_vf",
...
class TimePositionalEncoding(tf.keras.layers.Layer):
    """Adds positional information along the time dimension."""
    def __init__(self, seq_len: int, d_model: int, mode: str = "learned", dropout: float = 0.0, **kwargs):
        super().__init__(**kwargs)
        self.seq_len = int(seq_len)
        self.d_model = int(d_model)
        self.mode = str(mode).lower()
        self.dropout = float(dropout)
        self._drop = tf.keras.layers.Dropout(self.dropout) if self.dropout and self.dropout > 0 else None
        self._pos = None
    ...
    def build(self, input_shape):
        if self.mode in ("none", "off", "disabled"):
            self._pos = None
            return
        if self.mode in ("learned", "trainable"):
            self._pos = self.add_weight(
                name="pos_emb",
                shape=(self.seq_len, self.d_model),
                initializer=tf.keras.initializers.RandomNormal(stddev=0.02),
                trainable=True,
            )
        elif self.mode in ("sinusoidal", "sincos", "sin"):
            import numpy as _np
            pe = _np.zeros((self.seq_len, self.d_model), dtype=_np.float32)
            position = _np.arange(self.seq_len, dtype=_np.float32)[:, None]
            div_term = _np.exp(_np.arange(0, self.d_model, 2, dtype=_np.float32) * (-_np.log(10000.0) / self.d_model))
            pe[:, 0::2] = _np.sin(position * div_term)
            pe[:, 1::2] = _np.cos(position * div_term)
            self._pos = tf.constant(pe, dtype=tf.float32)
        else:
            raise ValueError(f"Unknown time_pos mode='{self.mode}' (use none/learned/sinusoidal)")
...
def build_model(
    ...
    head_mlp: bool = False,
    head_mlp_dim: int | None = None,
    var_drop_path: float | None = None,
    quantiles: Optional[List[float]] = None,
    cls_heads: Optional[List[str]] = None,
    price_heads: Optional[List[str]] = None,
    no_price_head: bool = False,
):
    ...
    if arch == "patchtst":
        ...
        for _ in range(max(1, var_layers)):
            if var_drop_path is None:
                var_dp = 0.0
            else:
                var_dp = float(var_drop_path)
            y = ITransformerBlock(seq_len=seq_len, d_model=d_model, heads=heads, dropout=dropout)(x)
            delta = y - x
            if var_dp > 0:
                delta = DropPath(var_dp)(delta)
            x = x + delta
        x = tf.keras.layers.Dense(d_model)(x)
        if time_pos and time_pos != "none":
            x = TimePositionalEncoding(seq_len=seq_len, d_model=d_model, mode=time_pos, dropout=pos_dropout, name="time_pos")(x)
    ...
    def _head_dense(h, out_dim: int, name: str):
        if not head_mlp:
            return tf.keras.layers.Dense(out_dim, name=name)(h)
        dim = int(head_mlp_dim) if head_mlp_dim else int(d_model * 2)
        z = tf.keras.layers.Dense(dim, activation="gelu", name=f"{name}_mlp")(h)
        z = tf.keras.layers.Dropout(dropout, name=f"{name}_mlp_do")(z)
        return tf.keras.layers.Dense(out_dim, name=name)(z)

    # outputs
    outputs = {}
    if not no_price_head:
        if price_heads:
            if quantiles:
                raise ValueError("quantiles are not supported with price_heads")
            for head in price_heads:
                outputs[head] = _head_dense(h, 1, name=head)
        else:
            # price head (regression or quantiles)
            if quantiles:
                price_out = _head_dense(h, len(quantiles), name="price_q")
            else:
                price_out = _head_dense(h, 1, name="price")
...
def cost_aware_weight(y_true, cost: float, min_weight: float):
    # Emphasize samples whose absolute move exceeds cost (approx. break-even)
    abs_y = tf.abs(y_true)
    rel = (abs_y - cost) / (abs_y + 1e-6)
    rel = tf.clip_by_value(rel, 0.0, 1.0)
    return min_weight + (1.0 - min_weight) * rel


def cost_aware_huber(delta: float, cost: float, min_weight: float = 0.1):
    base = tf.keras.losses.Huber(delta=delta, reduction=tf.keras.losses.Reduction.NONE)

    def loss_fn(y_true, y_pred):
        w = cost_aware_weight(y_true, cost, min_weight)
        return tf.reduce_mean(base(y_true, y_pred) * w)
...
def make_window_dataset(X: np.ndarray, y_price: Optional[np.ndarray], y_cls: np.ndarray,
                        ...
                        price_key: Optional[str] = "price"):
    # X: (N,F) float32
    X_tf = tf.convert_to_tensor(X, dtype=tf.float32)
    y_price_tf = tf.convert_to_tensor(y_price, dtype=tf.float32) if y_price is not None else None
    y_cls_tf = tf.convert_to_tensor(y_cls, dtype=tf.int32)
    if sample_weight is None:
        sample_weight = np.ones_like(y_cls, dtype=np.float32)
    if sample_weight_cls is None:
        sample_weight_cls = np.ones_like(y_cls, dtype=np.float32)
    ...
    def map_fn(i):
        ...
        return x_seq, {"cls": yc}, {"cls": wc}

    ds = ds.map(map_fn, num_parallel_calls=tf.data.AUTOTUNE)
    ds = ds.batch(batch_size, drop_remainder=False).prefetch(tf.data.AUTOTUNE)
    return ds

def make_window_dataset_multi(X: np.ndarray, y_price: Optional[np.ndarray], y_cls_dict: dict,
                              end_indices: np.ndarray, seq_len: int, batch_size: int,
                              shuffle: bool, sample_weight: Optional[np.ndarray] = None,
                              sample_weight_cls: Optional[np.ndarray] = None,
                              price_key: Optional[str] = "price", y_price_dict: Optional[dict] = None,
    ...
    if sample_weight_cls is None:
        sample_weight_cls = np.ones((len(X),), dtype=np.float32)
    w_tf = tf.convert_to_tensor(sample_weight, dtype=tf.float32)
    w_cls_tf = tf.convert_to_tensor(sample_weight_cls, dtype=tf.float32)
    ...
    def map_fn(i):
        ...
        y = {}
        wdict = {}
        if y_price_dict and price_key is not None:
...
def main():
    ...
    ap.add_argument("--price-loss", choices=["huber", "mse", "logcosh", "mae", "cost_huber"], default="huber")
    ap.add_argument("--huber-delta", type=float, default=1.0)
    ap.add_argument("--cost-bps", type=float, default=15.0,
                    help="Round-trip cost in bps used for cost-aware price loss.")
    ap.add_argument("--cost-aware-min-weight", type=float, default=0.1,
                    help="Minimum loss weight for samples below cost threshold.")
    ...
    ap.add_argument("--var-drop-path", type=float, default=None, help="Optional DropPath for ITransformer blocks")
    ap.add_argument("--time-pos", choices=["none", "learned", "sinusoidal"], default="none",
                    help="Time positional encoding for time-attention")
    ap.add_argument("--pos-dropout", type=float, default=0.0, help="Dropout after time positional encoding")
    ap.add_argument("--head-mlp", action="store_true", help="Use 2-layer MLP in price heads")
    ap.add_argument("--head-mlp-dim", type=int, default=None)
    ...
    ap.add_argument("--no-price-head", action="store_true",
                    help="Disable price head entirely (classification-only model).")
    ...
    if multi_horizons is None:
        if args.num_classes > 1:
            if args.target_col == "tb_label":
                y_cls = df[args.target_col].astype(int).to_numpy().reshape(-1)
            elif args.target_col in ("label_3cls", "label_3cls_vf", "label_3cls_vf2", "target_dir", "target_dir_vf", "target_dir_vf2"):
                y_cls = _map_dir_to_3cls(df[args.target_col].to_numpy()).reshape(-1)
            else:
                raise ValueError(f"Multi-class mode requires tb_label/label_3cls/target_dir style target, got '{args.target_col}'")
        else:
    ...
    price_clip_meta = {}
    if price_multi_horizons and not args.no_price_head:
        if multi_horizons is None:
    ...
    if args.price_col and y_price is not None:
    ...
    if multi_horizons is None and not price_multi_horizons:
        ds_train = make_window_dataset(
            ...
            shuffle=True, sample_weight=sample_weight_price, sample_weight_cls=sample_weight_cls,
            price_key=None if args.no_price_head else price_key
        )
        ds_val = make_window_dataset(
            X, y_price, y_cls, val_ends, args.seq_len, args.batch_size,
            shuffle=False, sample_weight=sample_weight_price, sample_weight_cls=sample_weight_cls,
            price_key=None if args.no_price_head else price_key
        )
    ...
    model = build_model(
        ...
        no_price_head=args.no_price_head,
        time_pos=args.time_pos,
        pos_dropout=args.pos_dropout,
        head_mlp=args.head_mlp,
        head_mlp_dim=args.head_mlp_dim,
        var_drop_path=args.var_drop_path,
    )
    ...
    if quantiles:
        ...
        def make_price_metrics():
            return [tf.keras.metrics.MeanAbsoluteError(name="mae")]
    elif args.price_loss == "cost_huber":
        cost = float(args.cost_bps) / 10000.0
        price_loss = cost_aware_huber(
            delta=float(args.huber_delta),
            cost=cost,
            min_weight=float(args.cost_aware_min_weight),
        )
        def make_price_metrics():
            return [tf.keras.metrics.MeanAbsoluteError(name="mae")]
    else:
    ...
    if price_heads and not args.no_price_head:
        price_keys = list(price_heads)
    elif not args.no_price_head:
    ...
    if not price_keys:
        model.compile(optimizer=opt, loss={"cls": cls_loss}, metrics={"cls": cls_metrics})
    elif cls_heads:
